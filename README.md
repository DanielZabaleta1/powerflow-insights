...
| Silent caching vs. labeling cached answers | **Labeled** — a small "Cached example" badge on the 4 suggested questions, never shown on a live answer | The demo distinguishes cached vs. live answers so it never misrepresents what was computed on the spot. Same principle as the reinterpretation finding above: an unlabeled shortcut that looks identical to the real thing is worse than a labeled one, even when the shortcut itself is honest (real SQL, real verified numbers — see `src/lib/cachedAnswers.ts`). |
